...
const MultiplayerPage = ({ onBack, userName, playerInventory }) => {
  ...
  useEffect(() => {
    ...
    multiplayerSocket.connect({ username: userName }).then(() => {
      ...
      unsubscribe = multiplayerSocket.onOnlinePlayers((list) => {
      ...
      // Proactively request the current list after subscribing
      console.log('[MultiplayerPage] Requesting initial online players list');
  ...
  multiplayerSocket.setStatus('in-game');
    ...
    setTimeout(() => {
      if (isQueuing) {
        setIsQueuing(false);
        alert(`Match found! You're playing against a ${selectedGameMode === 'competitive' ? 'ranked' : 'casual'} opponent.`);
      }
    }, matchTime);
  };
